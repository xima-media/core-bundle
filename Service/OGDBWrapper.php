...
class OGDBWrapper implements ContainerAwareInterface
{
    ...
    {
        $filename = $this->container->getParameter('kernel.cache_dir').'/'.self::OGDB_LOCAL_DATA_FILE_NAME;

        if (!is_readable($filename)) {
            $fileData = file_get_contents(self::OGDB_REMOTE_DATA_FILE);
            if ($fileData == false) {
                die('ABBRUCH: konnte Daten nicht laden ('.self::OGDB_REMOTE_DATA_FILE.")\n");
            }
